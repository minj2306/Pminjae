...
public class Ex3_예제 {
	...
	public static void main(String[] args) {
		...
		//Map< String , Integer> map = new Hashmap<>(); //2000개 저장이 안되거나 오류 발생 [ 비동기 = 여러 스레드가 동시에 한 메소드에 요청할때 대기 없으면 ]
		
		//2. //----- 멀티스레드를 이용한 각 1000 개씩 2000개를 동시 저장
		...
		// main 스레드는 다른 두 스레드가 작업(저장) 이 끝날때 까지 main 스레드 잠시 대기
		try { threadA.join(); threadB.join(); } catch (Exception e) { }
		
		System.out.println( "map 객체수 : " + map.size() );
		System.out.println( "map 상태 : " + map ); // 두 스레드가 저장을 하기 전에 main 스레드 출력 먼저 했기 때문에
