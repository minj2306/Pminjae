...
public class Driver {
	...
	public void drive( int value , Vehicle vehicle ) {//매개변수 타입과 자식타입 포함
		//매개변수에 부모객체와 다양한 자식 객체들이 자동타입 변환해서 매개변수 들어온 상태
		...
		System.out.println("vehicle 타입 확인 : " + (vehicle instanceof Taxi));
			...
			//*강제 타입 변환 1. 태생 자식객체(자동타입변환으로 부모객체된 객체) 2.캐스팅
			//Bus bus2 = (Bus)vehicle; //오류발생
			if(vehicle instanceof Bus) {
				System.out.println("버스전진"); Bus bus =(Bus)vehicle;
			}
			if(vehicle instanceof Taxi) {
				System.out.println("택시전진"); Taxi taxi =(Taxi)vehicle;
			}
