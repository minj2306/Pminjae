...
public interface RemoteControl {//인터페이스 선언
	...
	// 구성멤버
		...
		// 2. *public 추상메소드	[ public abstract ] : 선언부 => 구현객체 O
		void turnOn();
		public abstract void turnOff();
		void setVolume( int volume );
		
		// 3. 디폴트 메소드 [ public ] default : 선언부 + 실행부 => 구현객체O
		...
		// 6. private 정적 메소드 : 디폴트메소드 , 정적메소드 호출 가능 
		private static void staticCommon() { System.out.println("정적메소드"); }
}

/*
 	인스턴스 멤버	: 객체 생성시 메모리 할당되는 필드 , 메소드 [static 없는]
 	정적 멤버		: 객체가 없어도 메모리 우선 할당된 필드 , 메소드 [static 있는]
 		- 정적멤버는 인스턴스 멤버에 접근할 수 없다. 
 */
