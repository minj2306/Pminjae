...
public class Board {
	...
	//1. 필드	: 객체의 데이터 저장하는 곳
		/*
		 -필드 종류
		 	- 인스턴스필드		: (힙)객체 생성시 객체안에 저장되어 있는 필드 - 사용
		...
		/*
			...
			- 생성자 선언
		 	...
		 	- 생성자 역할
		 		...
		 		- 2. 유효성 검사할때 / 계산식이 필요한 값을 필드에 저장할때
		 
		 		
		 	- 생성자 종류[ 오버로딩으로 인해 다양하게 선언 가능 ]
		 		- 오버로딩 : 식별자명 매개변수의 개수,순서,타입에 따라서 식별 가능한 것
		 	
		 	- 접근제한 private , public , default , p
		 
		 	- 기타 키워드
		 		- this			: (생략가능) 현재 클래스의 구성멤버 식별할때 사용
		 		- this.필드명		: 외부로부터 들어온 매개변수명괴 현재 클래스 내 필드명과 같을떄 필드 실별할 때
		 		- this()		: 현재 클래스의 생성자 호출
	...
	/*
	 	...
	 	- 메소드 선언
	 		접근제한자 리턴타입 메소드명 (타입 매개변수 , 타입 매개변수){ }
	 			- 접근제한자	: public , private , public , protected
	 	...
	 	- 메소드 사용목적
	 		...
	 		- 2. 재활용 : 코드 줄이기 / 클린 코딩
	 		- 3. 매개변수에 따른 서로 다른 실행/결과 반환할수 있다.
	...
	/*
	 	...
	 	- MVC 디자인 패턴 : 기능 역할에 따른 구분
	 		...
	 		M : 데이터모델과 실제 데이터
	 			...
	 			2. 하나의 객체로 만들어서 이동
	 			DTO : 데이터 이동 객체 [ 서로 다른 레이어간 이동할때 객체 사용 ]
	 				- DTO
						member	: ????회원 객체인데 회원 무슨 객체????
