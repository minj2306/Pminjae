	...
	JS는 HTML 에서 포함된 문서
		...
		- 사용방법 = JS에서 객체 제공 = 키와 값으로 이루어진 객체 
			...
			localStorage : 쿠키 
				...
				localStorage.clear()
		-JSON : JS 객체 문법으로 구조화된 데이터를 표현하기 위한 문자기반의 포멧[형식]
			- 구조(모양)는 객체/배열 인데 포멧[형식/자료형/타입] 은 문자
			-
				JS { id : 'qwe' , pwd : '1234'}
				JSON : "{ id : 'qwe' , pwd : '1234'}" 
			- 사용방법
				1. JSON.parse( )			: 문자타입을 객체/배열 타입 으로 변환
...
function onWrite(){
	...
	// 3. 해당 객체/변수 를 배열에 저장한다.
		//1. 기존에 쿠키에서 게시물들이 저장된 게시물 배열 호출한다.
	let boardList = JSON.parse( localStorage.getItem('boardList') ) ;
		//2. 만얃레 쿠키가 존재하지 않으면 [숫자 없다는걸 0 / 문자 없다는걸 골백/ 객체 없다는걸 null 표기]
	...
	boardList.push(board)
		//4. boardList를 다시 쿠키에 저장한다. *저장할때 문자 포멧으로 저장
	localStorage.setItem('boardList' , JSON.stringify( boardList ) )	
		//5. 확인 
	console.log( JSON.stringify( localStorage.getItem('boardList') ) )
		//6. 글쓰기 성공 했으면 페이지 전환
	alert('글쓰기 성공');
		//HTML	: <a href="경로"> </a>
		//JS	: location.herf="경로"
	...
	console.log( 1 )								//숫자 1
	console.log( sessionStorage.getItem('세션') )	//숫자 1 저장했지만 문자 1로 출력됨
	console.log( localStorage.getItem('쿠키') )		//숫자 2 저장했지만 문자 2로 출력됨 
}
