/*
	...
	변수 : 데이터 1개 저장 할 수 있는 공간/메모리/상자
	
	상수 : 데이터 1개 저장 할 수 있는 공간/메모리/상자
		선언키워드 변수명 = 저장할 데이터
	
	배열 : 데이터 여러개를 저장 할 수 있는 공간/메모리/상자
		- 순서대로 번호가 붙인 데이터들의 연속적인 형태
		- 순서대로 번호 = '인덱스' **
		...
		3. 배열 선언 : 
 		...
 		7. 배열내 데이터 추가
 				...
 				배열명.splice(위치인덱스, 0 , 새로운값) : 해당위치 인덱스에 새로운 값 저장
 					-특정 위치/인덱스
 					[*추가된 요소 뒤로 한칸씩 자동 밀려남]
 					
 		8. 배열내 데이터 삭제 
 				배열명.pop() : 배열내 마지막 위치 데이터 삭제
 				배열명.splice (제거할 인덱스, 제거 개수)
 				ex)
 		...
 		9. 배열내 데이터를 이용한 데이터 삭제
 				- 배열명.indexOf('찾을데이터') : 찾을데이터가 존재하면 찾은 인덱스 번호 아니면 -1
...
// 예제5 : 배열에 요소/데이터 추가
배열명.push('체리'); // push('체리') : 배열내 마지막 요소/ 데이터 위에 '체리' 추가
console.log(배열명)
배열명.splice(0, 0, '복숭아') //0번째 인덱스에서 0개 자르고 '복숭아' 추가
console.log(배열명) 

// 예제 6 : 배열에 특정 요소/데이터 삭제
