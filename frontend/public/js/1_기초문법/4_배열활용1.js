...
function 추가함수(){
	...
	// 1. input에서 입력받은 데이터 가져오자...
	let snameInput = document.querySelector(".sname")
		//document : HTML(DOM객체)
		//query : 쿼리(질의)
		//Selector : 선택자 (id , class , 마크업 등)
			//1. document.querySelector(".sname") : sname 이라는 class 명 가진 input 객체 호출
	...
	// 4. 배열 출력
		//document.write(학생리스트) //문서에 해당데이터를 쓰기 [기존데이터 사라짐]
	 	// 1. ul 객체 호출
...
function 삭제함수(){
	...
	let index = 학생리스트.indexOf(name)
		//해당 데이터가 배열에 존재하면 0~ 찾을 번호 없으면 -1
